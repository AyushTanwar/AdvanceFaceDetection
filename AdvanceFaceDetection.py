...
def boxmaker(net, frame, conf_threshold=0.7):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > conf_threshold:
            a1 = int(detections[0, 0, i, 3] * widthofFrame)
            b1 = int(detections[0, 0, i, 4] * heightofFrame)
            a2 = int(detections[0, 0, i, 5] * widthofFrame)
            b2 = int(detections[0, 0, i, 6] * heightofFrame)
            bboxes.append([a1, b1, a2, b2])
            cv.rectangle(opencvframe, (a1, b1), (a2, b2), (255, 255, 255), int(round(heightofFrame/150)), 2)
...
capture = cv.VideoCapture(args.input if args.input else 0 )
padding = 20
